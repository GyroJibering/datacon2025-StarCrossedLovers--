...
我们综合运用了多种密码猜测技术，构建了一套覆盖离线分析和在线攻击场景的攻击框架，最终获得全国二等奖。
...
我们从 **离线哈希破解** 和 **在线定向猜测** 两个维度展开攻击，涵盖了从传统统计模型到最先进的大语言模型（LLM）的多种方法。

整个项目不仅是一次竞赛的记录，更是一次对密码生成模式、用户行为习惯以及如何利用个人可识别信息（PII）进行精准攻击的深度研究。
...
1. 混合策略：没有任何单一模型能解决所有问题。传统规则与现代模型的结合才能实现覆盖率和准确率的最大化。
2. 数据质量：针对特定场景（如国外用户），使用匹配的数据集进行模型训练至关重要。

3. 后处理价值：对模型输出进行智能筛选和融合，能以较小成本显著提升破解率。



未来展望：

- 自动化攻击流水线：将数据清洗、模型训练、密码生成和结果融合等步骤整合成一个自动化的CI/CD流水线。
- 更先进的LLM应用：探索更大规模的LLM及更高效的微调方法（如QLoRA），以挖掘更深层次的密码模式。
- 可解释性研究：深入分析模型生成的密码，理解其决策依据，反向指导更优的密码安全策略。